...
def heappop_adj(open_set, fscore, gscore, priority, num_tie, times, sec_tie, depth_dic):
    ...
    return open_set, node, num_tie, sec_tie
...
def A_star(graph, start, goal, priority):
    ...
    last_f, last_h, last_depth, count_tie, second_tie, counter_space = 0, 0, 0, 0, 0, 0
    while open_set:
        open_set, min_node, count_tie, second_tie = heappop_adj(open_set, fscore, gscore, priority, count_tie, time_nodes, second_tie, depth_dict)

        current = min_node  # set that node to current
        if current == goal:
            return reconstruct_path(came_from, current), counter_space, len(closed_set), count_tie, second_tie
...
def write_to_csv(size, p, num_evaluated, time_per_node, num_of_ties, num_steps, overall_time, sec_ties, inx, space_):
...
def main(size, priority, exp_num):
    ...
    try:
        ...
        shortest_route, space, num_expand, num_ties, num_sec_ties = A_star(graph, start, destination, priority)
